...
def sort_dir(dir_path):
    ...
    if dir_path:
        ...
        desktop_path = get_desktop_path()
        output_dir = os.path.join(desktop_path, dir_name)
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
    if output_dir:
        file_dict = {}
...
root.geometry("360x150")  # Set the window size
...
btn_sort.grid(row=0, column=0, padx=10, pady=10)

btn_sort_dir = ctk.CTkButton(frame, text="Sort Folder", command=sort_directory)
btn_sort_dir.grid(row=0, column=1, padx=10, pady=10)

btn_merge = ctk.CTkButton(frame, text="Merge Folder", command=merge_directory)
btn_merge.grid(row=1, column=0, columnspan=2, padx=10, pady=10)
